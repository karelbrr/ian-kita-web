...
│   └── prepare-hero.mjs           # Builds the hero photo: crops the watermark strip, inpaints the wall logo
├── src/
│   ├── assets/
│   │   ├── logo.svg          # Official logotype (vector, extracted from the press-pack PDF)
│   │   ├── hero-stage-clean.jpg    # Hero background, generated by scripts/prepare-hero.mjs
...
The hero background is derived from `photos/machac-2026-stage.jpg` by `node scripts/prepare-hero.mjs`, which crops off the festival watermark and removes the small logotype shown on the LED wall (PatchMatch inpainting), so the site's own logotype is the only one in the hero.
